...
<script>
    ...
    !function () {
        ...
        table.render({
            elem: '#project-table',
            url: getRootPath() + '/user2/findAllProjectByProjectName?projectName='+ window.location.href.split("?")[1].split("=")[1], // 数据接口
        ...
        table.on('toolbar(project-filter)', function (obj) {
            ...
            switch (obj.event) {
                case 'search':
                    window.location.href = getRootPath() + "/user2/searchProject.jsp?projectName=" + formData.projectName;
                    break;
                default:
                    break;
            }
        ...
        table.on('tool(project-filter)', function (obj) { // tool 是工具条事件名，user-filter 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if (layEvent === 'detail') {
                window.location.href = getRootPath() + '/user2/projectDetail.jsp?projectId=' + data.projectId;
            }
        });
    }();
</script>
