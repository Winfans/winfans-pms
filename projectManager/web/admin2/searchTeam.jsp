    ...
    <div class="layui-body">
        ...
        <script type="text/html" id="team-left-toolbar">
            <form class="layui-form" lay-filter="team-form-filter">
                ...
                <a class="layui-btn layui-btn-normal" lay-event="add">添加成员</a>
...
<script>
    ...
    !function () {
        ...
        table.render({
            elem: '#team-table',
            url: getRootPath() + '/admin2/findAllProjectUserByProjectName?projectName='+ window.location.href.split("?")[1].split("=")[1], // 数据接口
        ...
        table.on('toolbar(team-filter)', function (obj) {
            ...
            switch (obj.event) {
                case 'add':
                    window.location.href = getRootPath() + "/admin2/insertTeam.jsp";
                    break;
                case 'del':
                    if (data.length === 0) {
                        layer.msg('请选择一行或者多行');
                        ...
                        layer.confirm('求求你不要删除我们行么？', {
                            ...
                            window.location.href = getRootPath() + "/admin2/batchDeleteProjectUser?userIdList=" + userIdList + "&projectIdList=" + projectIdList;
                        });
                    }
                    break;
                case 'search':
                    window.location.href = getRootPath() + "/admin2/searchTeam.jsp?projectName=" + formData.projectName;
                    break;
                default:
                    break;
            }
        ...
        table.on('tool(team-filter)', function (obj) { // tool 是工具条事件名，user-filter 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if (layEvent === 'detail') {
                window.location.href = getRootPath() + '/admin2/teamDetail.jsp?projectId=' + data.projectId + "&userId=" + data.userId;
                ...
                layer.confirm('求求你不要删除我行么？', {
                    ...
                    layer.close(index);
                    // 向服务端发送删除指令
                    window.location.href = getRootPath() + "/admin2/deleteProjectUserByProjectIdAndUserId?projectId=" + data.projectId + "&userId=" + data.userId;
                });
            } else if (layEvent === 'edit') {
                window.location.href = getRootPath() + "/admin2/updateTeam.jsp?projectId=" + data.projectId + "&userId=" + data.userId;
            }
        });
    }();
</script>
